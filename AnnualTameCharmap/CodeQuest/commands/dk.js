...
module.exports = {
    ...
    async execute(message, args) {
        try {
            ...
            // Check if player already exists in data file first
            const existingPlayer = await playerManager.getPlayer(userId, message.author.username);
            
            // Check if player was banned
            if (existingPlayer.banned) {
                return this.showBannedAccount(message, existingPlayer);
            }

            // Strict check: if player has createdAt, they already have an account
            if (existingPlayer.createdAt) {
                logger.info(`User ${message.author.username} (${userId}) tried to register but already has account`);
                return this.showExistingAccount(message, existingPlayer);
            }

            // Only truly new players (no createdAt) can proceed to registration
    ...
    async showExistingAccount(message, player) {
        const Realms = require('../models/Realms');
        const realms = Realms.getRealms ? Realms.getRealms() : Realms;
    ...
    async showSuccessfulRegistration(message, player) {
        ...
        const successEmbed = new EmbedBuilder()
            ...
            .addFields(
                {
                    name: '🌟 Tài khoản mới',
                    value: `**Tên:** ${player.username}\n**ID:** ${player.userId}\n**Cảnh giới:** ${realms[player.realm].name}\n**Tạo lúc:** ${new Date().toLocaleString('vi-VN')}`,
                    inline: true
                },
                {
                    name: '💪 Stats ban đầu',
    ...
    getRealmProgress(player) {
        ...
        const progress = ((player.experience - currentRealm.baseExp) / (nextRealm.baseExp - currentRealm.baseExp)) * 100;
