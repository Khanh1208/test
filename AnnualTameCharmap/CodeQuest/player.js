...
class PlayerService {
    ...
    // Get player data (without auto-creation)
    async getPlayer(userId, username = '') {
        ...
        if (!player) {
            // Return empty player template without saving to file
            const emptyPlayer = this.createDefaultPlayer();
            emptyPlayer.userId = userId;
            emptyPlayer.username = username;
            // Don't set createdAt - this indicates it's not a real account yet
            delete emptyPlayer.createdAt;
            return emptyPlayer;
        } else {
            // Update last active and username for existing players
    ...
    // Create a real player account (called only during registration)
    async createPlayerAccount(userId, username) {
        if (!this.initialized) {
            await this.initializePlayerData();
        }
        ...
        await this.savePlayersData();
        logger.info(`👤 Tạo tài khoản mới: ${username} (${userId})`);
...
module.exports = {
    ...
    createPlayerAccount: (userId, username) => playerService.createPlayerAccount(userId, username),
